 ...
 * @FieldFormatter(
 *   id = "marketo_link",
 *   label = @Translation("Marketo Link"),
 *   field_types = {
 *     "marketo_form"
 *   }
 * )
 ...
 */
class MarketoLinkFormatter extends FormatterBase implements ContainerFactoryPluginInterface {
   ...
   *   Render array.
   */
  protected function buildLink(MarketoForm $item, EntityInterface $entity) {
    ...
    // Create class from plugin id.
    $class = 'marketo-btn--' . str_replace('_', '-', $current_form->getMarketoFormType()->id());

    // Entity type:
    $bundle = $entity->bundle();

    // If field = Marketo Hdr Link, and Bundle = Regional Pages:
    if ($field_name == 'field_shared_marketo_link' && $bundle == 'regional_pages') {
      // Use hash for URL:
      $url = '#show-modal';
    }
    else {
      // Use the actual URL:
      $url = $current_form->getPageUrl();
    }

    // Build the link.
    if ($current_form->hasField('field_shared_cta_button_text') && $current_form->get('field_shared_cta_button_text')->value) {
        $element = Link::fromTextAndUrl(
          $current_form->get('field_shared_cta_button_text')->value,
          Url::fromUserInput($url)
        );
    } else {
        $element = Link::fromTextAndUrl(
          $current_form->get('field_shared_button_text')->value,
          Url::fromUserInput($url)
      );
    }
