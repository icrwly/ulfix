...
/**
 * Access controller for the Legal Hold entity.
 *
 * @see \Drupal\ul_marketo\Entity\MarketoForm.
 */
class LegalHoldAccessControlHandler extends EntityAccessControlHandler implements EntityHandlerInterface {
  ...
  protected function checkAccess(EntityInterface $entity, $operation, AccountInterface $account) {
    /** @var \Drupal\ul_legal_hold\Entity\LegalHoldInterface $entity */
    switch ($operation) {
      case 'view':
        $access = AccessResult::allowedIfHasPermission($account, 'view legal holds');
        return $access;

      case 'create':
        $access = AccessResult::allowedIfHasPermission($account, 'add legal hold');
        return $access;

      case 'update':
        return AccessResult::allowedIfHasPermission($account, 'edit legal holds');

      case 'delete':
        return AccessResult::allowedIfHasPermission($account, 'remove legal hold');
