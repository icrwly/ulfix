...
# a different $db_role.
internal_domain="$4"
drush_cmd="drush9 --root=${docroot} --uri=${domain}"
profile=`${drush_cmd} php-eval 'print drupal_get_profile();'`
...
blt_args=""
case "${profile}" in
    ul_guidelines_profile|ul_enterprise_profile)
        # Add bogus cm.strategy arg so updb runs but no other config import strategy runs.
        blt_args="-D cm.core.key=${profile}"
        echo "BLT args are $blt_args"
        ;;
esac
...
# Execute the updates.
DRUSH_PATHS_CACHE_DIRECTORY="$cache_dir" $blt drupal:update --environment=$env --site=${name[0]} --define drush.uri=$internal_domain $blt_args --verbose --no-interaction
result=$?
