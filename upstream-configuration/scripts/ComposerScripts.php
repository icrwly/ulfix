...
class ComposerScripts {
  ...
  /**
   * Collect and write all composer patches into root composer.patches.json file.
   */
  public static function writeComposerPatchFile(PreCommandRunEvent $event) {
    $ulPath = 'upstream-configuration/patches.ul.json';
    $customPath = 'custom-dependencies/patches.custom.json';
    $ulArray = json_decode(file_get_contents($ulPath), true);
    $customArray = json_decode(file_get_contents($customPath), true);
    $combinedArray['patches'] = $ulArray + $customArray;
